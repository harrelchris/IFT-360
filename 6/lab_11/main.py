...
for t in clean_text:
    ...
    # add every four successive words to the list of quadgrams
    quad = zip(words[:-3], words[1:-1], words[2:-2], words[3:])
    # print(list(quad))
    for q in quad:
        x = ' '.join(q)
        quadgrams[x] = 0
        quadgram_cnts[x] = quadgram_cnts.get(x, 0) + 1
        # add entry to four-word suggestion list
        if quadgram_cnts[x] == 1:
            k = q[0] + " " + q[1] + " " + q[2]
            if k in three_wrd_sug:
                (three_wrd_sug[k]).append(x)
            else:
                three_wrd_sug[k] = [x]

#print some sample words for which a suggestion can be made
three_wrds=['the gypsii mobile', 'london marketwatch share', 'operating profits in',
            'and fixed infrastructure', 'for return on', 'understand the decision']
print("These are some of the three-word phrases that the agent can suggest a word after:")
for i in three_wrds:
    print(i)
print("==================================")
